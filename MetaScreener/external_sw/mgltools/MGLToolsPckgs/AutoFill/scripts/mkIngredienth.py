...
def computeMesh(mol, resolution=-0.1, smooth=16):
    ...
    coarseMesh = coarseMolSurface(coords, radii, [smooth,smooth,smooth],
                                  ...
                                  padding=0.0, name=mol.name+'_CoarseSurface')
...
def computeSpheresRange(minNbSph=1, maxNbSph=4, resolution=-0.1, smooth=16):
    ...
    for i in range(first, maxNbSph):
        if sys.platform=='win32':
            ...
            pgm = '/mgl/ms1/people/sanner/src/spheretree-dist-1.0/src/makeTreeMedial'
        os.system('%s -depth 2 -branch %d tmpCoarseMesh.obj'%(pgm,i))
...
def computeSpheres(nbSph=4, nbModels=3, resolution=-0.1, smooth=16):
    ...
    if sys.platform=='win32':
        ...
        pgm = '/mgl/ms1/people/sanner/src/spheretree-dist-1.0/src/makeTreeMedial'
    os.system('%s -depth %d -branch %d tmpCoarseMesh.obj'%(pgm,nbModels, nbSph))
...
def saveSpheres1(filename, spheres, radii):
    f = open(filename, 'w')
    for c, rad in zip(spheres, radii):
        print (c[0], c[1]+3., c[2], rad)
        f.write("%.2f %.2f %.2f %.2f\n"%(c[0], c[1], c[2], rad))
    f.close()
...
if __name__=='__main__':
    ...
    #exple 4 1 : one model with 4 spheres
    #mini = int(sys.argv[2])
    #maxi = int(sys.argv[3])

    from time import time
    t1 = time()
    print 'nba', len(mols[0].allAtoms)
    ...
    sphg = showApprox(vi, spheres, rads, coarseMesh)
