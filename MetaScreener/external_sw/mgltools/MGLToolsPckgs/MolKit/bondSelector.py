...
#
# $Header: /opt/cvs/python/packages/share1.5/MolKit/bondSelector.py,v 1.28 2013/09/23 19:39:16 rhuey Exp $
#
# $Id: bondSelector.py,v 1.28 2013/09/23 19:39:16 rhuey Exp $
#
#
#
...
class AromaticCycleBondSelector2(BondSelector):
    ...
    def select(self, bnds, cutoff=7.5):
        ...
        for ring in rf.rings:
            blist = ring['bonds']
            len_blist = len(blist)
            ring_elements = set(AtomSet(ring['atoms']).element)
            #SF 6/2013
            #verify that coplanarity
            if (len_blist<5):
                continue 
            if len_blist==5:
                if not ('N' in ring_elements) and not ('S' in ring_elements) and not ('O' in ring_elements):
                    continue            
            for bnd in blist:
                at = bnd.atom1
...
class RotatableBondSelector(BondSelector):
    ...
    def select(self, bnds, detectAll=False):
        ...
        #print "found ", len(rotatable), " rotatable bonds"
        nonrotatable = BondOrderBondSelector().select(bnds,2)
        #print "found ", len(nonrotatable), " bond order 2 bonds"
        if len(nonrotatable):
            for b in nonrotatable:
                if b in rotatable:
                    rotatable.remove(b)                   
        #select the leaves 
        #print "now there are ", len(rotatable), " rotatable bonds"
        leafBonds = LeafBondSelector().select(rotatable)
        #print "found ", len(leafBonds), " leaf bonds"
        rotatable = rotatable - leafBonds
        #print " now there are ", len(rotatable), " rotatable bonds"
        #select the cycles 
        cycleBonds = CycleBondSelector().select(rotatable, detectAll=detectAll)
        #print " found ", len(cycleBonds), " cycleBonds"
        #remove the cycleBonds
        if len(cycleBonds):
            if len(cycleBonds)==len(rotatable):
                rotatable = BondSet()
            else:
                for b in cycleBonds:
                    try:
                        ...
                        pass
                        #print "problem removing ", b, " from rotatable (line 564)"
        #return remaining bonds:
        return rotatable
