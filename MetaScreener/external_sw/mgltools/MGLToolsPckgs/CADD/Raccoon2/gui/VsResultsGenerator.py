...
import time
import os, sys
from CADD.Raccoon2.HelperFunctionsN3P import getCoords, pmvAtomStrip, getLines, quickdist, dist, percent, getAtype # TODO
...
class VSDockingResultsGenerator:
    ...
    def getInteractSummary(self, pose=0, recOnly=False):
        ...
        if recOnly:
            ...
            if pose['pi']['p-stack']:
                for n, ppi in enumerate(pose['pi']['p-stack']):
                    res, coordL, coordR = ppi
                    pose['pi']['p-stack'][n] = [ res, list(coordL), list(coordR) ]
                    print "\n\nXXX", pose['pi']['p-stack'][n] 
                interactions['ppi'] = [ pose['pi']['p-stack'] ]
                #print "\n\n======================\n%s\n===================\n\n" % interactions['ppi']
            if pose['pi']['t-stack']:
                for n, ppi in enumerate(pose['pi']['t-stack']):
                    res, coordL, coordR = ppi
                    pose['pi']['t-stack'][n] = [ res, list(coordL), list(coordR) ]
...
class AutoDockVinaVsResult(VSDockingResultsGenerator):
    ...
    def generatePDBQTplus(self):
        ...
        return buff
###
###    def getInteractSummary(self, pose):
###        """ collect nd format the interaction information for the pose"""
###        interactions = {}
###        if pose.hasattrib('hba'):
###            interactions['hba'] = [ pmvAtomStrip(p[0]), pmvAtomStrip(p[1]) for p in pose['hb']['acceptors'] ]
###        if pose.hasattrib('hbd'):
###            interactions['hbd'] = [ pmvAtomStrip(p[0]), pmvAtomStrip(p[1]) for p in pose['hb']['donors'] ]
###        if pose.hasattrib['vdw']:
###            interactions['vdw'] = [ pmvAtomStrip(p) for p in pose['vdw_contacts'] ]
###        if pose.hasattrib['metal']:
###            interactions['metal'] = [ pmvAtomStrip(p[0]), pmvAtomStrip(p[1]) for p in pose['metal_coord'] ]
###        if pose.hasattrib['ppi']:
###            interactions['ppi'] = [ pose['pi']['p-stack'] ]
###        if pose.hasattrib['tpi']:
###            interactions['tpi'] = [ pose['pi']['t-stack'] ]
###        return interactions
###
###
