...
#########################################################################
#
# $Header: /opt/cvs/DejaVu2/Qt/Camera.py,v 1.4 2014/07/18 14:50:47 sanner Exp $
#
# $Id: Camera.py,v 1.4 2014/07/18 14:50:47 sanner Exp $
#
import numpy
...
class Camera(QtOpenGL.QGLWidget, StandardCameraBase):
    ...
    def __init__(self, master, screenName, viewer, num, check=1,
        ...
        if sharedContext is None and len(viewer.cameras):
             ...
             QtOpenGL.QGLWidget.__init__(self, parent=master, shareWidget=sharecontextWidget)
        else:
             QtOpenGL.QGLWidget.__init__(self, fmt, parent=master)
        self.setAutoBufferSwap(1)
...
##             Tkinter.Widget.__init__(self, self.frame, 'togl', cnf, kw)
    ...
    def zoomCamera(self, delta):
        ...
        by changing the FOV
        """
        fovy = self.fovy - self.fovy * .05 * delta
        if (fovy < 180.) and (fovy > 0.):
            self.Set(fov=fovy)

    def scaleSlab(self, delta):
        """change near and far clipping planes to make slab thicjer or thinner
        """
    ...
    def handleZoomCamera(self, x, y, dx, dy, e):
        self.zoomCamera( self.getDelta(dx,dy,e) )
