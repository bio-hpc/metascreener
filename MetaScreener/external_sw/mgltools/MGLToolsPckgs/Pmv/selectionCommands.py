...
# $Header: /opt/cvs/python/packages/share1.5/Pmv/selectionCommands.py,v 1.182 2014/07/31 15:50:20 rhuey Exp $
#
# $Id: selectionCommands.py,v 1.182 2014/07/31 15:50:20 rhuey Exp $
#
...
## class BeforeClearSelectionEvent(VFEvent):
##     pass

## class ClearSelectionEvent(VFEvent):
##     pass
...
class MVSelectCommand(MVCommand, MVAtomICOM):
    ...
    # do this in the dashboardCommands.py
    ...
    def negateCmdBefore(self, nodes, negate=False,
        ...
        if len(self.vf.selection):
            kw['only'] = 1
            return ( [(self, (self.vf.selection.copy(),), kw)], self.name)
        else:
            return ( [(self.vf.clearSelection, (), {})], self.name)
    ...
    def __call__(self, nodes, negate=False, only=False, klass=None, 
        ...
        if klass != self.vf.selectionLevel:
            self.vf.setSelectionLevel(klass, topCommand=0, callListener=0)
...
"""
    ...
    def highlightSelection(self):
        ...
        boundGeoms = []
        if self.vf.commands.has_key('DisplayBoundGeom'):
            boundGeoms = self.vf.DisplayBoundGeom.getBoundGeomNames()
    ...
    def startICOM(self):
        ...
        self.vf.setSelectionLevel( self.vf.ICmdCaller.level.value)
...
class MVClearSelection(MVCommand):
   ...
   """
    ...
    def negateCmdBefore(self):
        ...
        if len(self.vf.selection):
            return ( [(self.vf.select, (self.vf.selection.copy(),), {})],
                     self.vf.select.name)
...
class MVSelectSetCommand(MVCommand):
    ...
    def negateCmdBefore(self, name, negate=False, only=False):
        # we overwrite negateCmdBefore enven though this command implements
        ...
        select = self.vf.select
        if len(self.vf.selection)==0:
            return ( [(self.vf.clearSelection, (), {})], self.vf.clearSelection.name)
        else:
            return ( [(self.vf.select, (self.vf.selection.copy(),), {})],
                              self.vf.select.name )
...
class MVSelectFromStringCommand(MVCommand):
    ...
    def negateCmdBefore(self, *args, **kw):
        ...
        cmds = ( [(self.vf.clearSelection ,(), {})], self.name)
        if len(oldselection):
            cmds[0].append( (self.vf.select ,(oldselection,), {}) )
        return cmds
    ...
    #                self.vf.setSelectionLevel(selitem[0].__class__, busyIdle=0, log=0)
            self.vf.select( selitem , negate=negate, xor=xor,
                            intersect=intersect, topCommand=0, setupNegate=False)
            #if not negate:
    ...
    def getAtomSetsVal(self, event=None):
        ...
        for i in self.setsVar.keys():
            if i not in specialCases:
                ...
                if lev == Protein: lev = Molecule
                self.vf.setSelectionLevel(lev, busyIdle=0, log=0)
...
class MVSelectSphericalRegion(MVCommand):
    ...
    def negateCmdBefore(self,  *args, **kw):
        select = self.vf.select
        oldselection = self.vf.selection.copy()
        return ([(self.vf.select, (oldselection,),
                  {'only':True, 'topCommand':0})], self.name)
...
class MVInvertSelection(MVCommand):
    ...
    def negateCmdBefore(self,  *args, **kw):
        select = self.vf.select
        oldselection = self.vf.selection.copy()
        return ( [(self.vf.select, (oldselection,), {'only':True, 'topCommand':1})],
                 self.name)
...
##             if len(old)==0:
##                 self.vf.setSelectionLevel(all[0].__class__, busyIdle=0, log=0)
...
##                 self.vf.select(all-old, topCommand=0)
        ...
        self.vf.deselect(old & allobjects, topCommand=0, setupNegate=False)
        self.vf.select(allobjects - old, topCommand=0, setupNegate=False )
...
class SelectNoWaterHeteroAtomsCommand(MVSelectFromStringCommand, MVSelectCommand):
    ...
    def doit(self, nodes, negate=False, only=False, xor=False, intersect=False):
        ...
        self.vf.setSelectionLevel(Atom, topCommand=0, callListener=0)
        if only:
...
class MVExpandSelection(MVCommand):
    ...
    def negateCmdBefore(self,  *args, **kw):
        select = self.vf.select
        oldselection = self.vf.selection.copy()
        return ( [(self.vf.select, (oldselection,), {'only':True, 'topCommand':0})],
                 self.name)
