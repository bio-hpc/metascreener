...
#
# $Header: /opt/cvs/python/packages/share1.5/Pmv/displayCommands.py,v 1.226 2013/10/03 22:08:11 annao Exp $
# 
# $Id: displayCommands.py,v 1.226 2013/10/03 22:08:11 annao Exp $
#
...
"""
    ...
    def getNodes(self, nodes):
        ...
        molecules.\n
        This function is used to prevent the expansion operation to be done in both doit and negateCmdBefore The nodes.findType( Atom ) is the operation that is potentially expensive.\n
        """
...
##         self.managedGeometries.append(p)
    ...
    def negateCmdBefore(self, nodes, only=False, negate=False, displayBO=False,
                        lineWidth=2, **kw):
        #print "negateCmdBefore", self.name, "negate:", negate, "kw", kw
...
class DisplayCPK(DisplayCommand):
    ...
    def onAddCmdToViewer(self,):
        ...
        if self.vf.hasGui:
            ...
            self.unitedRadii = Tkinter.IntVar()
            self.unitedRadii.set(1)
    ...
    def negateCmdBefore(self, nodes, only=False, negate=False, scaleFactor=1.0,
                        cpkRad=0.0, quality=0, byproperty = False,
                        propertyName = None, propertyLevel = 'Molecule',
                        setScale=True, unitedRadii=True ):

        #print "in negateCmdBefore: only= ", only, "negate=", negate, "scaleFactor=", scaleFactor, "cpkRad=", cpkRad, "quality=", quality, "property = ", propertyName, "propertyLevel = ", propertyLevel
    ...
    def doit(self, nodes, only=False, negate=False, 
             ...
             propertyName=None, propertyLevel='Molecule', setScale=True, unitedRadii=True, **kw):
    ...
    def __call__(self, nodes, only=False, negate=False, 
                 ...
                 propertyLevel = 'Molecule', setScale=True, unitedRadii=True,  **keywords):
    ...
    def buildFormDescr(self, formName='default'):
        ...
        if formName == 'default':
            ...
            idf.append({'name': 'unitedRadii',
                        'parent':'radiiGroup',
                        'widgetType': Tkinter.Checkbutton,
                        'tooltip': 'allows the user to use United radii vs Regular radii.\nThe button is checked for United radii.',
                        'wcfg': {'text': 'United Radii',
                                 'variable': self.unitedRadii},
                        'gridcfg':{'sticky':'w'}} ) 
    ...
    def guiCallback(self):
        ...
        if val:
            ...
            kw['unitedRadii'] = val['unitedRadii']
            apply(self.doitWrapper, (nodes,), kw)
...
##                               })
            ...
            if form.descr.entryByName.get('propertyName'):
...
current selection cannot be colored using this function."
    ...
    def negateCmdBefore(self, nodes, property, only=False, negate=False, scaleFactor=1.0,
        ...
        if len(geomSet) == 0:
            ...
            # command is to undisplay what you just displayed 
            kw['negate'] = True
            return ( [(self, (nodes, property), kw)], self.name)
        else:
            # If the current was already displayed and only is False and negate
            # is False then the undo is to display this set using the last used
            # values.
            ...
            # command is to display ONLY what was displayed before.
            kw['only'] = True
            return ( [(self, (geomSet, property), kw)], self.name)


    def doit(self, nodes, property, only=False, negate=False, 
...
"""
    ...
    def doit(self, nodes, only=False, negate=False, bRad=0.3,
        ...
        def drawBalls(mol, atm, only, noBalls, negate, bRad, bScale,
            ...
            if isinstance(bquality, IntType):
...
"""
    def onAddObjectToViewer(self, obj):
        ...
        self.vf.browseCommands('displayCommands',
                               commands=('displaySticksAndBalls',), log=0,topCommand=0)
        self.vf.browseCommands('displayCommands',
                               commands=('displayLines',), log=0, topCommand=0)
        self.vf.browseCommands('secondaryStructureCommands',
                               commands=('displayExtrudedSS',), log=0, topCommand=0)
...
class DisplayInteractionsCommand(MVCommand):
    ...
    def negateCmdBefore(self, lig, macro):
        #???color???
        ...
        # The undo of a display command is to display ONLY what was
        # displayed before, if something was already displayed
        cmds.append( (self.vf.displayLines, (geomSet,), kw) )
        kw = {'log':0,'topCommand':0, 'redraw':True}
        old_colors = macro.findType(Atom).colors['lines']
        #old_colors = macro.allAtoms.colors['lines']
        cmds.append((self.vf.color, (geomSet,old_colors,), kw) )
        ...
        cmds.append((self.vf.labelByProperty, (geomSet,), kw) )
        kw = {'log':0,'topCommand':0, 'redraw':True}
        ...
        if len(geomSet):
            ...
            kw['negate'] = 1
        cmds.append((self.vf.displayCPK, (geomSet,), kw) )
        #macro ss
        res = self.getSSResidues(macro)
        if len(res):
             kw = {'log':0,'topCommand':0, 'redraw':True, 'negate':1}
             cmds.append((self.vf.ribbon, (res,), kw) )
        #kw = {'log':0,'topCommand':0, 'redraw':True, 'negate':1}
        ...
        if len(ballset)==0:
            ...
            if len(stickset) == 0: # negate
                ...
                kw['negate'] = True
                kw['redraw'] = True
                cmds.append((self.vf.displaySticksAndBalls,(lig.findType(Atom),), kw) )
            else:
                # noballs was on
                kw['negate'] = False
                kw['redraw'] = True
                kw['only'] = True
                cmds.append((self.vf.displaySticksAndBalls, (stickset,), kw) )
        else:
            kw['redraw'] = True
            kw['only'] = True
            cmds.append((self.vf.displaySticksAndBalls, (stickset,), kw)  )
        ...
        cmds.append((self.vf.displayMSMS, (lig,), {'surfName':surfName, 'negate':1}) )
        lgca = lig.geomContainer.atoms
        geomSet = lgca['cpk']
        kw = {'log':0,'topCommand':0, 'redraw':True}
        if len(geomSet):
            #undisplay all
            kw['negate'] = 1
            cmds.append((self.vf.displayCPK, (lig.allAtoms,), kw))
        ...
        if len(displayed_res):
            ...
            cmds.append( (self.vf.displayExtrudedSS, (displayed_res,), kw))
        else:
            kw = {'log':0,'topCommand':0, 'redraw':True, 'negate':True}
            cmds.append( (self.vf.displayExtrudedSS, (macro.chains.residues,), kw))
        old_color = self.vf.GUI.VIEWER.currentCamera.backgroundColor
        if len(old_color)>3: old_color = list(old_color[:3])
        cmds.append( (self.vf.setbackgroundcolor, [old_color,], {}) )
        cmds.append( (self.restoreCpk, (lig, macro), {}))
        cmds.append( (self.close, (), {}))
        cmds.append( (self.vf.hbondsAsSpheres.dismiss_cb, (), {}))
        cmds.append( (self.vf.setbackgroundcolor.dismiss_cb, (), {}) )

        return ( cmds, self.name)
