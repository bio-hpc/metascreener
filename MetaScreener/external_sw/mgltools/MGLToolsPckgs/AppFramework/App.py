...
#########################################################################
#
# $Header: /opt/cvs/AppFramework/App.py,v 1.21 2014/07/18 00:03:04 annao Exp $
#
# $Id: App.py,v 1.21 2014/07/18 00:03:04 annao Exp $
#

"""
define a base class AppFramework that support lazy loading of commands
"""
import os, thread, weakref, traceback
...
class AppFramework:
    ...
    """
        ...
        ## first try to create a gui witout imoprting command
    ...
    def addObjectType(self, typeName, typeValidator, events):
        ...
        for key in ['startAddObject', 'addObjectCmd', 'endAddObject',
                    'beforeDeleteObject', 'afterDeleteObject']:
            assert events.has_key(key)
            assert issubclass(events[key], Event)
    ...
    def addObject(self, name, obj, objType, geomContainer=None):
        ...
        event = events['startAddObject'](name=name, object=obj)
        self.eventHandler.dispatchEvent(event)
        ...
        for com in self.onAddObjectCmds[objType]:
            ...
            event = eventClass(number=i, total=l, type=objType,
                               cmdName=com[0].name)
            self.eventHandler.dispatchEvent(event)
            i += 1
        # create add object event
        event = events['endAddObject'](object=obj)
        self.eventHandler.dispatchEvent(event)
