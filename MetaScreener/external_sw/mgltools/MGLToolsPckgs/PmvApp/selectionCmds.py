...
# $Header: /opt/cvs/PmvApp/selectionCmds.py,v 1.10 2014/08/15 22:57:44 annao Exp $
#
# $Id: selectionCmds.py,v 1.10 2014/08/15 22:57:44 annao Exp $
#
...
class selectionFeedbackEvent(Event):
    pass
...
class MVSelectCommand(MVCommand): #, MVAtomICOM):
    ...
    def __init__(self):
        ...
        #self.app().setSelection(TreeNodeSet())
    ...
    def undoCmdBefore(self, nodes, negate=False,
                        only=False, klass=None, xor=False,
                        intersect=False):
        kw={}
        frepStr = formatName(nodes.buildRepr(), 60)
                
        if negate:
            name = 'select %s'%frepStr
        else:
            name = 'deselect %s'%frepStr

        if self.app().activeSelection.empty():
            return ( [(self.app().clearSelection, (), {})], 'select %s'%frepStr)
        else:
            kw['only'] = 1
            return ( [(self, (self.app().activeSelection.atoms.copy(),), kw)], name)


    def checkArguments(self, nodes, negate=False, only=False, klass=None, 
        ...
        Optional Arguments:\n
            ...
            only  --- if  True,  the current selection is replaced by the nodes \n
            klass ----if None, the nodes class is used, else\n
                  nodes is converted to a set of objects of type Klass and\n
                  current selection level is set to klass
            intersect ---
    ...
    def doit(self, nodes, negate=False, only=False, xor=False, intersect=False, klass=None):
        ...
        old = app.activeSelection.get()
        if klass is None: klass = nodes[0].__class__
        if klass != app.selectionLevel:
            app.setSelectionLevel(klass)

        selAtoms = nodes.findType(Atom)

        if only:
            setOn = nodes
            setOff = app.activeSelection.get() - selAtoms
            app.setSelection(selAtoms)

        elif not app.activeSelection.empty(): # something was selected before
            if negate: # nodes will be unselected
                setOn = []
                setOff = nodes
                app.setSelection(app.activeSelection.get() - selAtoms)

            elif xor: # selected elements of node will be deselected and
                      # unselected will be selected
                setOff = app.activeSelection.get() & nodes
                setOn = nodes
                app.setSelection(app.activeSelection.get() ^ selAtoms)
            elif intersect:
                setOff = app.activeSelection.get() ^ nodes
                setOn = app.activeSelection.get() & nodes
                app.setSelection(app.activeSelection.get() & selAtoms)
            else: # or
                setOff = []
                setOn = nodes
            ...
            setOff = []
            setOn = nodes
            app.setSelection(selAtoms.copy())
        else:
            setOff = setOn = []
            
        # if the selection level was set on an empty selection without
        # specifying a KlassSet we have to set the proper class here
        #if len(self.activeSelection) > 0:
            #obj = self.activeSelection[0]
            #if self.activeSelection.__class__ != obj:
                #app().setSelection(obj.setClass(self.activeSelection.data))

        ## if not app.activeSelection.empty(): 
        ##     #remove possible duplicates
        ##     newVal = app.activeSelection.uniq()
        ##     if len(newVal)<len(app.activeSelection):
        ##         # in this case duplicates are removed
        ##         # but selection is no longer ordered
        ##         app.setSelection(newVal)

        # The GUI part of the application should register listeners (with callbacks) for the following events:
        event = selectionFeedbackEvent(objects=app.activeSelection)
        app.eventHandler.dispatchEvent(event)
        # The listener of this event should implement and register
        # a method the does the following:
        #self.updateSelectionFeedback()
        #self.highlightSelection()

        new = app.activeSelection.get()
        event = SelectionEvent(app.activeSelection, new=new, old=old,
                               setOn=setOn, setOff=setOff)
        app.eventHandler.dispatchEvent(event)
        return app.activeSelection.atoms
...
## !! The following code should go into the gui part of the command !!
    ...
    def clear(self):
        ...
        #event = selectionFeedbackEvent(objects=self.app().activeSelection)
...
class MVDeSelectCommand(MVSelectCommand):
    ...
    Required Arguments:\n       
        ...
        klass --- if specified nodes are converted to a set of that type, \n
                  and selection level is set to klass.
    """
    def checkArguments(self, nodes, klass=None, only=False, negate=False):
        """ nodes --- TreeNodeSet holding the current selection \n
...
class MVClearSelection(MVCommand):
   ...
   """
    ...
    def doit(self):
        setOff = self.app().activeSelection.get().copy()
        app = self.app()
        if hasattr (app.select, "loadCommand"):
...
class MVSelectSphericalRegion(MVCommand):
    ...
    def doit(self, centerList, radius, selList=['all']):
        ...
        if hasattr(self.app(), 'logAllFile'):
            self.app().logAllFile.write("from numpy.oldnumeric import array, float32\n")
...
class MVInvertSelection(MVCommand):
    ...
    def undoCmdBefore(self,  *args, **kw):
        select = self.app().select
        oldselection = self.app().activeSelection.atoms.copy()
        return ( [(self.app().select, (oldselection,), {'only':True})],
                 self.name)
    ...
    def doit(self, invertLevel=None, subset=None):
        ...
        if invertLevel is not None:
            if invertLevel == 'all':
                if level==Atom:
                    allobjects = self.app().Mols.allAtoms
                else:
                    allobjects = self.app().Mols.findType(level)

            elif invertLevel == 'molecule':
                if self.app().activeSelection:
                    mols = sel.top.uniq()
                    allobjects = sel[0].setClass()
                    for mol in mols:
                        allobjects = allobjects + mol.findType(level)
                                                   
##             all = self.app().Mols.allAtoms

##             if invertLevel == 'all':
##                 all=self.app().Mols.findType(lev)
...
##             if len(old)==0:
##                 self.app().setSelectionLevel(all[0].__class__, busyIdle=0, log=0)
##                 #self.app().setIcomLevel(all[0].__class__, 
##                     #log = 0, KlassSet = all[0].setClass)
##                 self.app().select(all)
##             else:
##                 self.app().deselect(old)
##                 self.app().select(all-old)
       
        elif subset is not None:
            allobjects = subset.findType(level)

        if allobjects == None or len(allobjects) == 0: 
            msg = '%s: No molecules!' % self.name
            self.app().warningMsg(msg)
            return 
    ...
    def checkArguments(self, invertLevel=None, subset=None):
        """
        \ninvertLevel --- 'all' or 'molecule'
        \nsubset      --- set of atoms, residues, chains or molecules"""
        ...
        if isinstance(subset, str):
            subset = self.app().expandNodes(subset)
        return (), {'invertLevel':invertLevel, 'subset':subset }
...
class MVExpandSelection(MVCommand):
    ...
    def doit(self, selection, centers, dist, keys):
        self.app().selectInSphere(centers, dist, keys)
        self.app().select(selection)

    def checkArguments(self, selection , centers, dist, selList=['all']):
        """centers --- specifies the centers of the selection spheres \n
        dist  --- 
        selList --- specifies selection base (atoms to test)\n
                possible selList values are ['all'], a list of sets or a list\n
                of molecule names. 
        """
        assert selection
        assert isinstance(centers, (list, tuple, numpy.ndarray))
        assert len(centers)
        assert isinstance(dist, (int, float))
        assert isinstance(selList, (list, tuple))
        assert len(selList)
        return (selection, centers, dist,selList), {}
