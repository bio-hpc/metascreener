#!/usr/bin/env /opt/MGLTools-1.5.6/bin/pythonsh
#
# 
#
...
def remove_alt_locations(m):
    counter = [0, 0]
    for chain in mol.chains:
        for residue in chain.residues:
            have_altloc = [atom for atom in residue.atoms if atom.altname]

            counter[1] += len(have_altloc) > 0
            counter[0] += len(have_altloc)

            for atom in have_altloc:
                if atom.altname == 'A':
                    atom.altname = ''
                    atom.name, _ = atom.name.split('@')
                else:
                    residue.remove(atom)
                    m.allAtoms.remove(atom)

    if counter[0]:
        print('**** Alternate locations found: removed %d alternate atoms from %d residues' % tuple(counter))
...
if __name__ == '__main__':
    ...
    def usage():
        ...
        print
        print "** WARNING: this script will remove atoms with alternate locations higher than 'A'"
        print "            and clear altLoc on 'A' alternate locations.                (Ric 2016)"
        print 
    ...
    # Remove alt locations -- Ric 2016
    remove_alt_locations(mol)
